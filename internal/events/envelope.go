...
// evidence.package.v1 and evidence.validation.v1 Kafka topics. Every
// envelope carries a FHIR R4 message Bundle entry and a provenance block
// binding the event to its principal and payload signature.
package events
...
const (
	EnvelopeVersion      = "1.0"
	Producer             = "maritime-evidence"
	ClassificationIntern = "INTERNAL"

	// TopicPackage carries evidence package receipt events.
	TopicPackage = "evidence.package.v1"
	// TopicValidation carries evidence validation decision events.
	TopicValidation = "evidence.validation.v1"
)

// validTopic reports whether the topic is a platform v1 contract topic.
func validTopic(topic string) bool {
	switch topic {
	case TopicPackage, TopicValidation:
		return true
	default:
		return false
	}
...
// aggregate (evidence package), payloadJSON is the domain payload, and
...
// key: alg=EdDSA, a maritime-evidence kid, an exact canonical-payload
