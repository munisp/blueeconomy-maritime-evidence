...
// {"alg":"EdDSA","kid":"maritime-evidence-<epoch>"}, where <epoch> is the
...
const (
	...
	SigningKeyEnv = "EVIDENCE_ENVELOPE_SIGNING_PRIVATE_KEY"
	// SigningKeyEpochEnv carries the decimal key-rotation epoch used in the
	// JWS kid ("maritime-evidence-<epoch>").
	SigningKeyEpochEnv = "EVIDENCE_ENVELOPE_SIGNING_KEY_EPOCH"

	signingAlgorithm = "EdDSA"
	keyIDPrefix      = "maritime-evidence-"
)
...
// space is not the maritime-evidence rotation epoch); platform producers
...
// alg=EdDSA with a maritime-evidence kid, the payload must re-canonicalize
...
// instead of the maritime-evidence rotation prefix. An empty prefix
// fails closed.
